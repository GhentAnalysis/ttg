...
def createSignalRegions(c):
  if c.njets == 1:
    if c.ndbjets == 0: return (0 if not c.isEMu else 5)
    if c.ndbjets == 1: return (1 if not c.isEMu else 6)
  elif c.njets >= 2:
    if c.ndbjets == 0: return (2 if not c.isEMu else 7)
    if c.ndbjets == 1: return (3 if not c.isEMu else 8)
    if c.ndbjets >= 2: return (4 if not c.isEMu else 9)
  return -1
...
if args.tag.count('randomConeCheck'):
  ...
  plots.append(Plot('signalRegions',              'signal region',                        lambda c : createSignalRegions(c),                             (10, 0, 10), histModifications=xAxisLabels(['1j,0b', '1j,1b','#geq2j,0b','#geq2j,1b','#geq2j,#geq2b','1j,0b', '1j,1b','#geq2j,0b','#geq2j,1b','#geq2j,#geq2b'])))
