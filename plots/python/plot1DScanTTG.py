...
if args.units == '':
    ...
    axishist.GetXaxis().SetTitle("%s %s" % (fixed_name, args.units.replace('-3','3')) )
...
if args.units == '':
    ...
    textfit = '%s = (%.3f{}^{#plus %.3f}_{#minus %.3f}) %s' % (fixed_name, val_nom[0], val_nom[1], abs(val_nom[2]), args.units.replace('\\','#'))
...
if args.breakdown is None:
    for i, other in enumerate(other_scans):
        if args.units == '':
            ...
            textfit = '#color[%s]{(%s = %.3f{}^{#plus %.3f}_{#minus %.3f}) %s}' % (other_scans_opts[i][2], fixed_name, other['val'][0], other['val'][1], abs(other['val'][2]), args.units.replace('\\','#'))
        pt.AddText(textfit)
...
if args.units == '':
    ...
    save_graph.GetXaxis().SetTitle('%s = (%.3f %+.3f/%+.3f) %s' % (fixed_name, val_nom[0], val_nom[2], val_nom[1], args.units))
