...
#     plotList.append(Plot('Z_pt',                  'p_{T}(ll) [GeV]',                   lambda c : Zpt(c),                                            (20, 10, 200)))
#     plotList.append(Plot('l1l2_ptsum',            'p_{T}(l1)+p_{T}(l2) [GeV]',         lambda c : c.l1_pt+c.l2_pt,                                   (20, 10, 300)))
...
#       plotList.append(Plot('photon_pt_ATLB',             'p_{T}(#gamma) [GeV]',                   lambda c : c.ph_pt,                                           [20., 30., 40., 50., 65., 80., 100., 125., 160., 220., 300.]))
    ...
    plotList.append(Plot('lhetop_mass',                    'm lhe top  [GeV]',                  lambda c : c.mlhetop,                                         (100, 162.5, 182.5 )))
    plotList.append(Plot('lheatop_mass',                   'm lhe anti-top  [GeV]',             lambda c : c.mlheatop,                                        (100, 162.5, 182.5 )))

    plotList.append(Plot('lhetop_massB',                    'm lhe top  [GeV]',                  lambda c : c.mlhetop,                                         (200, 162.5, 182.5 )))
    plotList.append(Plot('lheatop_massB',                   'm lhe anti-top  [GeV]',             lambda c : c.mlheatop,                                        (200, 162.5, 182.5 )))

    plotList.append(Plot('lhetop_massC',                    'm lhe top  [GeV]',                  lambda c : c.mlhetop,                                         (400, 162.5, 182.5 )))
    plotList.append(Plot('lheatop_massC',                   'm lhe anti-top  [GeV]',             lambda c : c.mlheatop,                                        (400, 162.5, 182.5 )))

    plotList.append(Plot('lhetop_massD',                    'm lhe top  [GeV]',                  lambda c : c.mlhetop,                                         (1000, 162.5, 182.5 )))
    plotList.append(Plot('lheatop_massD',                   'm lhe anti-top  [GeV]',             lambda c : c.mlheatop,                                        (1000, 162.5, 182.5 )))

    # if args.extraPlots.lower().count('topmass'):
      # NOTE not done
      # plotList.append(Plot('lhetop_mass',                    'm lhe top  [GeV]',                  lambda c : c.mll,                                              (20, 0, 200)))
  ...
  if not args.showSys and not copySyst and plotsToFill:
    ...
    for sample in sum(stack, []):
      ...
      for i in sample.eventLoop(''):
        ...
        # eventWeight = c.genWeight*c.puWeight*c.lWeight*c.lTrackWeight*c.phWeight*c.bTagWeight*c.triggerWeight*prefireWeight*lumiScale*c.ISRWeight*c.FSRWeight*c.PVWeight*estWeight*zgw
        
        # NOTE  zet trerug
        eventWeight = c.genWeight*c.puWeight*lumiScale


        # eventWeight = lumiScale

# TODO remove again, topmass testing
        # eventWeight = eventWeight* getBWrew(c, 172.5, 1.4915)  
        # eventWeight = eventWeight* getBWrew(c, 'up', 1.4915)  
        eventWeight = eventWeight* getBWrew(c, 'up', 1.4915)  
        # eventWeight = eventWeight* getBWrew(c, 172.5-0.4, 1.48445)  
