...
with open(args.input, 'r') as f:
  ...
  for line in lines:
    ...
    if dasResult:
      ...
      unsure = False
      if len(sampMatch) > 1:
        # favour PSweights sample if there is one available
        nameB = origName.replace('TuneCUETP8M2T4','TuneCP5').replace('TuneCUETP8M1','TuneCP5')
        if not nameB.count('PSweights'): nameB = nameB.replace('_TuneCP5','_TuneCP5_PSweights')
        sampMatch = difflib.get_close_matches(nameB, sampMatch, n=5)
        print 'WARNING: no perfect match for ' + name + ', chose:'
        print sampMatch[0]
        print 'alternatives:'
        for i in sampMatch[1:]:
          print i
        print(' ')
      chosen = sampMatch[0]
      chosenFull = [s for s in dasResult.split() if chosen in s]
      for s in chosenFull:
        hnList.write(s + '\n')
      ...
      for key in nameMapping.keys():
        if name.startswith(key):
          matches.append(key)
      if len(matches) ==1:
        xList.write(nameMapping[matches[0]][0].ljust(30) + (pnfsPath + chosen).ljust(150) + '@LABEL@    ' + str(nameMapping[matches[0]][1]).ljust(3) + '    ' + xsec + '\n')
      else:
        xList.write('@name me@'.ljust(30) + (pnfsPath + chosen).ljust(150) + '@LABEL@    ' + xsec + '\n')
