...
for fitFile, dist in distList:
  ...
  mcTot = pickle.load(open('/storage_mnt/storage/user/jroels/public_html/ttG/' + args.year + '/unfNov1/noData/placeholderSelection/' + dist.replace('unfReco','recFid_unfReco') + '.pkl','r'))[dist.replace('unfReco','recFid_unfReco')]['TTGamma_DilPCUTt#bar{t}#gamma (genuine)']
  mcTot.SetBinContent(mcTot.GetXaxis().GetNbins()+1, 0.)
  mcTot.SetBinContent(0, 0.)
  ...
  difc = plMC.Clone()
  difc.Add(unfoldedMC, -1.)
  l = [ difc.GetBinContent(i) for i in range(0, difc.GetXaxis().GetNbins()+1)]
  lr = [99 if unfoldedMC.GetBinContent(i)==0 else difc.GetBinContent(i)/unfoldedMC.GetBinContent(i) for i in range(0, difc.GetXaxis().GetNbins()+1)]
  log.info(l)
  log.info(lr)
