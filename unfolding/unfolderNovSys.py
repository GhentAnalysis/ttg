...
distList = [
# ('data/otra16AsrFit_fitDiagnostics_exp.root', 'unfReco_jetLepDeltaR'),
...
for fitFile, dist in distList:
  ...
  outMig = pickle.load(open('/storage_mnt/storage/user/jroels/public_html/ttG/' + args.year + '/unfNovSys/noData/placeholderSelection/' + dist.replace('unfReco','out_unfReco') + '.pkl','r'))[dist.replace('unfReco','out_unfReco')]['TTGamma_DilPCUTt#bar{t}#gamma (genuine)']
  rec = pickle.load(open('/storage_mnt/storage/user/jroels/public_html/ttG/' + args.year + '/unfNovSys/noData/placeholderSelection/' + dist.replace('unfReco','rec_unfReco') + '.pkl','r'))[dist.replace('unfReco','rec_unfReco')]['TTGamma_DilPCUTt#bar{t}#gamma (genuine)']
  ...
  response = pickle.load(open('/storage_mnt/storage/user/jroels/public_html/ttG/' + args.year + '/unfNovSys/noData/placeholderSelection/' + dist.replace('unfReco','response_unfReco') + '.pkl','r'))[dist.replace('unfReco','response_unfReco')]['TTGamma_DilPCUTt#bar{t}#gamma (genuine)']


  plMC = pickle.load(open('/storage_mnt/storage/user/jroels/public_html/ttG/' + args.year + '/unfNovSys/noData/placeholderSelection/' + dist.replace('unfReco','fid_unfReco') + '.pkl','r'))[dist.replace('unfReco','fid_unfReco')]['TTGamma_DilPCUTt#bar{t}#gamma (genuine)']
...
# 
  ...
  log.info(unfolded.GetBinContent(2))
  log.info(unfolded.GetBinError(2))
  ...
  for i in range(0, mcTotStat.GetXaxis().GetNbins()+1):
    if histsum.GetBinContent(i)==0:
      mcTotStat.SetBinError(i, histsum.GetBinError(i))
    else:
      mcTotStat.SetBinError(i, histsum.GetBinError(i)/histsum.GetBinContent(i)*mcTotStat.GetBinContent(i))

  unfold.SetInput(mcTot)
  unfold.DoUnfold(0.)
  unfoldedMCStat = unfold.GetOutput('unfoldedMCStat_' + dist)
  unfoldedMCStat.Scale(1./lumiScales[args.year])
...
# Draw the blue stat unc only band
  ...
  # unfMC1 = unfoldedMC.Clone()
  # for i in range(0, unfMC1.GetXaxis().GetNbins()+1):
  #   if histsum.GetBinContent(i)==0:
  #     unfMC1.SetBinError(i, histsum.GetBinError(i))
  #   else:
  #     unfMC1.SetBinError(i, histsum.GetBinError(i)/histsum.GetBinContent(i)*unfMC1.GetBinContent(i))

  # unfoldedMCStat.SetLineWidth(3)
  # unfoldedMCStat.SetFillStyle(3244)
  # unfoldedMCStat.SetLineColor(ROOT.kBlue)
  # unfoldedMCStat.SetFillColor(ROOT.kBlue)
  # unfoldedMCStat.Draw('E2 same')
