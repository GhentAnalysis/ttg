...
from ttg.tools.style import drawLumi, commonStyle, setDefault, setDefault2D
...
def loadYear(fitFile, year, responseVersion):
  ...
  outMig = pickle.load(open('/storage_mnt/storage/user/jroels/public_html/ttG/' + year + '/' + responseVersion + '/noData/placeholderSelection/' + dist.replace('unfReco','out') + '.pkl','r'))[dist.replace('unfReco','out')]['TTGamma_DilPCUTt#bar{t}#gamma (genuine)']
...
# labels = {
#           'unfReco_phPt' : ('phPt', 'PL phPt'),
#           'unfReco_phLepDeltaR' : ('phLepDeltaR', 'PL phLepDeltaR'),
#           'unfReco_phEta' : ('phEta', 'PL phEta'),
#           'unfReco_ll_deltaPhi' : ('phEta', 'phEta')
#           }
# setDefault()
...
# for dist in ['unfReco_phEta']:
  ...
  ########## loading ##########
  fitFile = 'data/3yearsrFit_fitDiagnostics_exp.root'
  responseVersion = 'unflmva8'

# TODO NOTE testing with resp matrix 16
  data16, mcTot16 = loadYear(fitFile, '2016', responseVersion)
  data17, mcTot17 = loadYear(fitFile, '2017', responseVersion)
  data18, mcTot18 = loadYear(fitFile, '2018', responseVersion)

  response16 = pickle.load(open('/storage_mnt/storage/user/jroels/public_html/ttG/2016/'+ responseVersion +'/noData/placeholderSelection/' + dist.replace('unfReco','response') + '.pkl','r'))[dist.replace('unfReco','response')]['TTGamma_DilPCUTt#bar{t}#gamma (genuine)']
  response17 = pickle.load(open('/storage_mnt/storage/user/jroels/public_html/ttG/2017/'+ responseVersion +'/noData/placeholderSelection/' + dist.replace('unfReco','response') + '.pkl','r'))[dist.replace('unfReco','response')]['TTGamma_DilPCUTt#bar{t}#gamma (genuine)']
  response18 = pickle.load(open('/storage_mnt/storage/user/jroels/public_html/ttG/2018/'+ responseVersion +'/noData/placeholderSelection/' + dist.replace('unfReco','response') + '.pkl','r'))[dist.replace('unfReco','response')]['TTGamma_DilPCUTt#bar{t}#gamma (genuine)']
  ...
  # respStichCanv = ROOT.TCanvas('stichedResp' + dist,'stichedResp' + dist, 1900, 700)
  ...
  # respR2.GetYaxis().SetTitleOffset(0.65)
  # respR2.GetXaxis().SetTitleOffset(1.45)
  ...
  # respR2.Draw('COLZ')
  # respStichCanv.SaveAs(dist + 'stichedResp.png')
  # break
