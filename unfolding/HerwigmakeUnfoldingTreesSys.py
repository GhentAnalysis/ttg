...
def getTotalWeightsHack(sample):
  ...
  for f in sample.listOfFiles:
    ...
    f = ROOT.TFile(f)
    try:
      totals += f.Get('maker/tree').GetEntriesFast()
      # log.info(totals)
    except:
...
from ttg.unfolding.PLobjectSelectionHack import PLselectLeptons, PLselectPhotons, PLgoodJets, PLbJets, PLmakeInvariantMasses, PLmakeDeltaR
#
# Start selection
#
...
##### PL selection and storing values #####
  ...
  newVars.genWeight    = c.weight*lumiWeights[0]
