...
from ttg.unfolding.PLobjectSelection import PLselectLeptons, PLselectPhotons, PLgoodJets, PLbJets, PLmakeInvariantMasses, PLmakeDeltaR, checkPLphMatch
#
# Start selection
#
...
# from ttg.plots.photonCategories import checkMatch

def checkMatch(c, n):
  if not n.checkMatch:                                          return True
  if n.genuine        and c._phTTGMatchCategory[n.ph] == 1: return True
  if n.misIdEle       and c._phTTGMatchCategory[n.ph] == 2: return True
  if n.hadronicPhoton and c._phTTGMatchCategory[n.ph] in (3, 6): return True
  if n.hadronicFake   and c._phTTGMatchCategory[n.ph] in (4, 7): return True
  if n.magicPhoton    and c._phTTGMatchCategory[n.ph] == 5: return True
  if n.mHad           and c._phTTGMatchCategory[n.ph] == 3: return True
  if n.mFake          and c._phTTGMatchCategory[n.ph] == 4: return True
  if n.unmHad         and c._phTTGMatchCategory[n.ph] == 6: return True
  if n.unmFake        and c._phTTGMatchCategory[n.ph] == 7: return True
  return False

def checkRec(c, n):
  if n.failReco:                                          return False
  if not c._phCutBasedMedium[n.ph]:                       return False
  if abs(c._phEta[n.ph]) > 1.4442:                        return False
  n.checkMatch, n.genuine = True, True
  n.misIdEle,n.hadronicPhoton,n.hadronicFake,n.magicPhoton,n.mHad,n.mFake,n.unmHad,n.unmFake,n.nonPrompt = False,False,False,False,False,False,False,False,False
  if not checkMatch(c, n):                                   return False
  if not n.ph_pt > 20:                                    return False
  if not n.ndbjets>0:                                     return False
  if not n.mll > 20:                                      return False
  if not (abs(n.mll-91.1876)>15 or n.isEMu):              return False
  if not (abs(n.mllg-91.1876)>15 or n.isEMu):             return False
  return True


########## FIDUCIAL REGION ##########
def checkFid(c, n):
  if n.failFid:                                                 return False
  if abs(c._pl_phEta[n.PLph]) > 1.4442:                             return False
  if not n.PLph_pt > 20:                                            return False
  if not n.PLndbjets>0:                                             return False
  if not n.PLmll > 20:                                              return False
  return True
...
##### reco selection and storing values #####
  ...
  if fid and reco:
    if not (checkRec(c, newVars) and checkFid(c, newVars)): continue
    all+=1.
    if checkPLphMatch(c, newVars):
      match +=1.
    try:
      log.info(match/all)
    except:
