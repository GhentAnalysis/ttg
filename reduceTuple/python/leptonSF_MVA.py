...
class LeptonSF_MVA:
  def __init__(self, year): 
    ...
    self.elSF     = getObjFromFile(os.path.expandvars(os.path.join(dataDir, 'topmva_eg_VLoose_' + year + '.root')), 'EGamma_SF2D')
    self.elSyst   = getObjFromFile(os.path.expandvars(os.path.join(dataDir, 'topmva_eg_VLoose_' + year + '.root')), 'sys')
    self.elStat   = getObjFromFile(os.path.expandvars(os.path.join(dataDir, 'topmva_eg_VLoose_' + year + '.root')), 'stat')
    assert self.muSyst
    assert self.muStat
    assert self.elSF
    assert self.elSyst
