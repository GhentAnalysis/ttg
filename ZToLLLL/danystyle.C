...
void setTDRStyle() {
  std::cout << " --- setting TDR style ---" << std::endl;
...
// For the statistics box:
  ...
  //tdrStyle->SetOptStat("em"); // To display the mean and RMS:   SetOptStat("mr");
  ...
  //  tdrStyle->SetStatX(0.99);
  //  tdrStyle->SetStatY(0.88);
...
// For the axis titles:
  ...
  //  tdrStyle->SetTitleXOffset(5.);
  //  tdrStyle->SetTitleYOffset(10.);
...
// Postscript options:
  ...
  Int_t newcols[260];
  const Int_t ncols = TColor::GetNumberOfColors();
  for(Int_t i=ncols; i>0; --i) {newcols[i-1] = TColor::GetColorPalette(ncols-i);}
  TColor::SetPalette(ncols, newcols);
